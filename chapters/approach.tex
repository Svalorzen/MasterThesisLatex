...
As discussed \ys{put section number here}, the POMCP algorithm can't be applied directly to $\rho$-POMDPs. In this Section we present a modification of the algorithm POMCP which is able to deal with a belief-based reward
...
that needs to be considered by the solution process \cite{cit:rpomdp}. \ys{can you re-phrase this statement, its very difficult to understand.}
...
\ys{the above para is confusing}
...
\ys{use a different symbol for estimate of entropy (say $\hat{H}$), the same goes for any other estimates that you might be using in any algorithm or equation}
...
particle $k$ \ys{what is $k$} insertion in a particle belief, we simply remove from our previous full estimate the
...
\end{algorithm}
\ys{indicate input and output of the algorithm ... also is there a step like 'for each particle $s$' missing from the algorithm?? -  same comment for the algorithm above}
...
the limit. \ys{cite paninski and pomcp here}
...
A second problem is dependent on how returns are calculated in POMCP. $Q(b,a)$ \ys{$\pi$ and $h$} is computed as the
...
\ys{policy is missing from the equation}
...
of each $\rho$ estimation back up the chain through $V$, as it happened in POMCP via samples. \ys{why does it not allow?} This
...
\ys{it can be bit confusion going back and forth between $\rho(b)$ and $\rho(b,a)$. Once you have defined $rho$ to be dependent only on $b$, stick to it or else the text does not come out as consistent}
