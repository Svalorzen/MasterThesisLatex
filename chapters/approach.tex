...
%% CHECK AGAIN WITH CODE CAN'T REMEMBER
...
\nl             N($h$) = N($h,a$) = V($h,a$) = F($h,a$) = 0\;
\nl             futureRew = 0\;
            }
        }
\nl     B(h) = B(h) $\bigcup \;\{ s' \}$\;
\nl     r = estimate reward from B(h)\;
\nl     N($h$) = N($h$) + 1\;
\nl     N($h,a$) = N($h,a$) + 1\;
\nl     F($h,a$) += $\frac{futureRew - F(h,a)}{N(h,a)}$\;
\nl     V($h,a$) = r + $\gamma$ * F($h,a$)\;
\nl     oldV = V($h$)\;
\nl     V($h$) = $\max_{a'} V(h,a')$\;
...
$\rho$-POMCP starts off as POMCP, by extracting a particle state from the root belief and simulating
