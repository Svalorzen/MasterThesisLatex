In this thesis we tackle the \ys{a}Active \ys{p}Perception task applied over multi-camera systems. \ys{rather say, active perception for resource allocation in multi-camera systems} In
particular, we try to track a single target \ys{you do multi-person tracking in your experiments, so no need to say just one target here} over an extended period of time, while trying to make an
optimal usage of available resources and overcome \ys{under} specific constraints, namely partial
observability, environment size and limited access to cameras.

In order to fullfill our goal we have modified \ys{Dont say modified, rather say, you identified shortcomings of existing approach and improved them for your purpose ...} an existing approach in order to fit our particular
...
\ys{this para is bit vague .. cant understand what are you trying to say.}
...
replicate results obtained by [], \ys{?} which show that planning for more than one timestep in a realistic
