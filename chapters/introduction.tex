\section{Active Perception} \ys{is there a need for this heading?} 

\ys{going over the introduction overall, I find it bit unconvincing. I think you main introduction starts from '1.2 Application'. The part '1.1 Active perception' is way too confusing and informal.} 

\ys{I think we have discussed this many times before, the general flow of the introduction should have been 
\begin{itemize}
...
\item generally collecting information is not an means to an end , but in the example we gave above the aim of the agent is only to perceive the information. these type of tasks are called active perception task, define active prception. .....
\end{itemize}
}

\ys{I think a majority of this points are covered in the introduction you have written, but they are not in the right order which makes it confusing and not convincing. However, there are also some points which are not covered. Please try to cover all this points, as they make a strong case for your research. }
...
\ys{this paragraph does not serve a good purpose as an opening paragraph. Most of the statements are informal and does not make sense.} 
...
Perception} task. \ys{remove the first sentence, it does not sound right.} Active Perception can be formalized as a task where a decision making entity
...
of the following factors: \ys{again unnecessary and confusing para}
...
\ys{this actually is a very important point that the agent has to tackly uncertainty and should definitely come before the definition of active perception. This point is basically the backbone of motivation of your problem, so use it well} 

In this work we present an autonomous system that tackles some \ys{why some, if we are tackling only some challenges, then mention only those} of the challenges that characterize
...
tests where solutions can be evaluated and may also produce real world value. \ys{this sounds like we were interested in solving some active perception task and hence we chose this multi-camera system as a testbed problem. That is not really the case. The original problem is that multi-camera systems suffer from resource constraints and this problem falls is interesting in itself, we handle it by modelling it as a pomdp. such type of problems where agent wants to perceive information are called active perception probelms. .... .} In this work we focus
...
\ys{this actually, with some modification, can form a really good first paragraph for introduction. }
...
\item[Energy, bandwidth and communication:] These are the most common constraints for multi-camera
    ...
    \ys{talk about why exactly energy bandwidth are resource constraints, where is computational cost as scarce resource?}
...
\ys{this heading seems unnecessary}
...
\ys{should be more like 'this problems can be conveniently modelled using pomdps, rather than 'oh there are various methods to address active percerption problem and we choose pomdp.'}. 
...
to achieve optimality, and provide a proof in Appendix \ref{ref:appendix_proof}. \ys{proof is a big word, generally they are associated with theorems, which we are not presenting. You can say a appendix provides convincing example case.}
...
\ys{I think the above 4 para are ok, but has too much information for introduction. Try to get rid of information that you might not want in introduction}
...
\ys{summarize your contributions better. Say, we model the problem using pomdp, we solve the pomdp using mcts, we extend mcts to belief-dependent rewards, provide detailed empirical analysis on simulated and real data.}

\subsection{Chapter Overview}

\ys{better name is organization rather than chapter overview. I dont think you need it here, you already have a contents table at the start. }
