...
In this work we will \ys{"will" not required} approach a general class of problems which is referred to as the Active
Perception task. In this task a decision making entity called \textit{agent}, subject to resource
constraints, needs to take actions in order to reduce some form of uncertainty \ys{uncertainty about what} within a particular
...
improve its knowledge. \ys{If you want to mention this in introduction, a more general way of writing it would be something like this - "In active perception task, the agent may or may not affect the state of the envirornment example : something" then give example for both type of task, then tell how your problem is related to first type of example}

Active Perception covers essentially every possible use of sensors, both physical and virtual. \ys{not sure what you want to say here }
...
\item[Scalability:] Real world Active Perception systems often have to deal with very sizable
    ...
    sensor deployments - as painlessly as possible. \ys{are we dealing with this "change in environment" issue?}
\item[Non-Myopicity:] We argue that an optimal Active Perception solution can need \ys{can need?} to take into
    account the information that the agent will be able to obtain in the future. \ys{how about? - need to take into account how environment might evolve in future?} This requires the
...
The Active Perception task is a very \ys{"very" not required} general abstraction that can be applied to multiple real world
...
adoption of multi-camera systems. These systems can be \ys{'are' instead of 'can be'} used for surveillance, real-time tracking and
...
\item[Number of Targets:] Each tracked target contributes exponentially in increasing the complexity
    ...
    \ys{If we are not solving this problem, there is no point of mentioning it}
\end{description}
...
its current knowledge of the environment, but also its future one. \ys{how about - while reasoning about all future possibilities?} We argue that while myopic
...
POMDPs are also \ys{'also' not needed} a widely explored topic in the decision theoretic literature, with known properties
...
\ys{I am not sure of message of this paragraph, I think Monte carlo methods enable the scaling, not all online planning methods have this property. It sounds like you are saying that}
...
\ys{explain this in more detail, specially the part, where you highlight that POMCP is a sampling based method, hence belief based reward are a challenge}
...
\begin{description}
    ...
    \item[Estimate based rewards:] We modify the POMCP algorithm in order to consider rewards as
        ...
        should not be mixed up. \ys{not sure what this means}
