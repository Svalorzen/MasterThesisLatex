...
\begin{multicols}{2}
    ...
    \main{\mcts{s, h}}{
        \KwData{State s, Horizon h}
\nl     \While{enough time is available}{
\nl         \simulate{tree root, s, 0}\;
        }
...
\nl     \KwRet r\;
    ...
    \simul{\simulate{n, s, d}}{
        \KwData{Node n, State s, Depth d}
\nl     select action $\tilde{a}$\;
\nl     ($s', r$) $\sim$ G(s,$\tilde{a}$)\;
\nl     \If{$d < $ horizon}{
\nl         \eIf{n($\tilde{a},s'$) is in the tree}{
\nl             futureRew = \simulate{$n(\tilde{a},s'), s', d+1$}\;
            }{
\nl             add n($\tilde{a},s'$) to the tree\;
\nl             futureRew = \rollout{s', d+1}\;
            }
\nl         r = r + $\gamma$ * futureRew\;
            }
\nl     increment seen counter for n($\tilde{a}$)\;
\nl     n($\tilde{a}$) value += $\frac{r-n(\tilde{a}) \text{ value}}{n(\tilde{a}) \text{ seen counter}}$\;
\nl     \KwRet r\;
    }
...
\nl     \KwRet r\;
    ...
    \simul{\simulate{n, s, d}}{
        \KwData{Node n, State s, Depth d}
\nl     select action $\tilde{a}$ with UCT\;
\nl     (s', o, r) $\sim$ G(s,$\tilde{a}$)\;
\nl     \If{$d < $ horizon}{
\nl         \eIf{n($\tilde{a},o$) is in the tree}{
    \nl             futureRew = \simulate{$n(\tilde{a},o), s', d+1$}\;
            }{
\nl             add n($\tilde{a},o$) to the tree\;
\nl             futureRew = \rollout{s', d+1}\;
            }
\nl         r = r + $\gamma$ * futureRew\;
            }
\nl     add $s'$ to the particle belief of $n$\;
\nl     increment seen counter for n($\tilde{a}$)\;
\nl     n($\tilde{a}$) value += $\frac{r-n(\tilde{a}) \text{ value}}{n(\tilde{a}) \text{ seen counter}}$\;
\nl     \KwRet r\;
    }
