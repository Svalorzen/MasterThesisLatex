...
each timestep the agent receives a single observation from the environment which is dependent \ys{on}from
...
for failed attempts. All these would influence the final behavior of the agent. \ys{The last three statement of this para are bit vague.}
...
Unfortunately, this implies that the agent is unable to reason about the present.\ys{I am not sure what you mean by this statement, the agent is able to reason about the present and hence take actions.} While the
...
\textit{sufficient statistic} of the state of the environment. \ys{cite}
...
that its value function is \textit{piecewise linear and convex} (PWLC). \ys{cite} This means that it is
