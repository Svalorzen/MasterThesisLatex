...
In order to compute a strategy to complete a task within an environment we need to model the
dynamics of the environment together with the task objectives. The framework we are going to use in
this work stems from the \textit{Markov Decision Process} (MDP), named after Andrey Markov.
...
\cite{cit:pomdp}, where the belief is now the state of the agent.
