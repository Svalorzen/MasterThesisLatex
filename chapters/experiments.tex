In this \ys{c}Chapter we compare performances of our method against a number of different baselines on a
...
RTBSS \ys{cite} is an online branch-and-bound solver \ys{cite branc and bound} for POMDPs, which enumerates every possible outcome from
a given state for a given horizon, and thus computes the true value function for that state and the
best action that can be performed. \ys{does not RTBSS use branch and bound to prune actions and then compute the value function? if it does explain it here} RTBSSb works in the same way, but it computes rewards using a
...
action space of the problem \cite{cit:rpomdp}. \ys{i think this is not limited to just max of belief} \ys{I think you need to explain this really clearly. Actually I think what you have written here is wrong. [7] is $rho$POMDP paper, they make it possible to solve belief based reward by drawing tangents to the belief based reward. They dont propose any change in action space. } 
\ys{In fact I think this description should not be here in experiments in the first place. The $rho$ POMDP method to solve for belief based reward should be in background. and the approach section already explains how you solve for max of belief and belief entropy.}In the new action space, each action represents at
...
\ys{I think it is good to highlight the fact that converting belief based reward to state based reward causes a computational overhead. But you should not refer to prediction action as there in no background about them in the thesis. Stick to $rho$POMDP. They dont have any prediction actions. Instead they have tangents. }
...
\ys{heading of the section should be just simulated data/environment}
...
episode the target starts in a random room, and the agent goal is to track it for 10 timesteps. \ys{no need for 10 time steps here} The
...
the technique used to apply it to \ys{a} Active \ys{p}Perception substantially increases the branching factor of
the decision tree.
...
\ys{I really cant understand this para, specially the last line. I think you should not use any terminology refering to prediction actions.. does IR stand for Information reward? Again, the thesis has nothing like that throughout, so its not easy for me to understand this. I suppose the committe will only find it more difficult to understand it. }
...
\ys{Why is it called the realisitic scenrio? Large Grid setting might be a better name}
...
useful. \ys{the explaination is quite vague}
...
each action.
\ys{this is interesting}
\ys{One major piece missing from experiments section is real data.  I think I gave you the data and the POMDP for it. I think its essential that you have some experiments on the real dataset. Whats the status on that? Apart from that I dont think you need any more experiments. These are more than enough.} 
